...
class HMIServerKaldiClient(AbstractHMIServer):
    ...
    def _determine_answer(self, description, grammar, target, is_preempt_requested):
        ...
        # Connect Gstreamer pipeline
        kaldi_app.asr.connect('hyp-word', kaldi_app.wait_for_sentence)
        kaldi_app.pipeline.set_state(Gst.State.PLAYING)

        while not is_preempt_requested and not kaldi_app.sentence and not rospy.is_shutdown():
